#!/usr/bin/env python3
#
# VENDORED, UNMODIFIED except for this header and the shebang above
# (/usr/bin/python3 -> env python3, so it also runs where python3 isn't
# at that exact path). Source: LLVM 21's own
# /usr/lib/llvm-21/share/clang/clang-tidy-diff.py.
#
# Why a copy lives in the repo at all: unlike run-clang-tidy, this script
# is NOT installed onto $PATH - it sits under a version-numbered
# directory (/usr/lib/llvm-<N>/share/clang/), so its location differs per
# machine and per CI image. Vendoring it makes `misc/clang-tidy-diff.py`
# mean the same thing everywhere.
#
# What it's for: running clang-tidy against ONLY the lines a diff
# touches, instead of the whole tree. That's what makes a naming/warning
# gate usable while a backlog still exists - new code is held to the
# rules from day one, and the existing findings get fixed as the files
# are touched anyway, rather than blocking everything behind one big
# cleanup. See CONTRIBUTING/CI for the exact invocation.
#
# Deliberately NOT vendored alongside it: run-clang-tidy. That one IS on
# $PATH already (/usr/bin/run-clang-tidy), so a second copy here would
# only risk drifting from the installed clang-tidy it drives.
#
